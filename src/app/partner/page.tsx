    ...
    <div className="min-h-screen bg-white text-slate-900">
          ...
          <div className="relative max-w-6xl mx-auto px-6 pt-24 pb-16 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
              ...
              <button onClick={() => setMode("login")}
                className="px-8 py-4 bg-white hover:bg-slate-50 border border-slate-200 text-slate-800 rounded-2xl font-semibold text-lg transition-all shadow-sm">
                Partner Login
              </button>
            </div>
      ...
      {mode === "register" && (
        <div className="max-w-2xl mx-auto px-6 py-20">
          <button onClick={() => { setMode("landing"); setError(""); }}
            className="text-slate-500 hover:text-slate-900 mb-8 flex items-center gap-2 transition-colors">
            ← Back
          </button>
          <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            <h2 className="text-3xl font-bold mb-2 text-slate-900">Join as Partner</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-slate-600 font-medium mb-1 block">Full Name *</label>
                  <input value={regForm.name} onChange={e => setRegForm({...regForm, name: e.target.value})}
                    placeholder="Rahul Sharma" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
                <div>
                  <label className="text-sm text-slate-600 font-medium mb-1 block">Phone *</label>
                  <input value={regForm.phone} onChange={e => setRegForm({...regForm, phone: e.target.value})}
                    placeholder="9876543210" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
              </div>
              <div>
                <label className="text-sm text-slate-600 font-medium mb-1 block">Email *</label>
                <input type="email" value={regForm.email} onChange={e => setRegForm({...regForm, email: e.target.value})}
                  placeholder="rahul@example.com" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
              </div>
              <div>
                <label className="text-sm text-slate-600 font-medium mb-1 block">Password *</label>
                <input type="password" value={regForm.password} onChange={e => setRegForm({...regForm, password: e.target.value})}
                  placeholder="Min 6 characters" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
              </div>

              <div className="border-t border-slate-100 pt-4">
                <p className="text-sm text-slate-500 mb-4">Payment Details <span className="text-slate-400">(optional — add later)</span></p>
                <div>
                  <label className="text-sm text-slate-600 font-medium mb-1 block">UPI ID</label>
                  <input value={regForm.upi_id} onChange={e => setRegForm({...regForm, upi_id: e.target.value})}
                    placeholder="rahul@upi" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                  <div>
                    <label className="text-sm text-slate-600 font-medium mb-1 block">Account Holder Name</label>
                    <input value={regForm.bank_holder_name} onChange={e => setRegForm({...regForm, bank_holder_name: e.target.value})}
                      placeholder="Rahul Sharma" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                  </div>
                  <div>
                    <label className="text-sm text-slate-600 font-medium mb-1 block">Bank Name</label>
                    <input value={regForm.bank_name} onChange={e => setRegForm({...regForm, bank_name: e.target.value})}
                      placeholder="SBI / HDFC etc." className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                  </div>
                  <div>
                    <label className="text-sm text-slate-600 font-medium mb-1 block">Account Number</label>
                    <input value={regForm.bank_account} onChange={e => setRegForm({...regForm, bank_account: e.target.value})}
                      placeholder="1234567890" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                  </div>
                  <div>
                    <label className="text-sm text-slate-600 font-medium mb-1 block">IFSC Code</label>
                    <input value={regForm.bank_ifsc} onChange={e => setRegForm({...regForm, bank_ifsc: e.target.value})}
                      placeholder="SBIN0001234" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                  </div>
                </div>
              </div>
      ...
      {mode === "login" && (
        <div className="max-w-md mx-auto px-6 py-20">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            <h2 className="text-3xl font-bold mb-2 text-slate-900">Partner Login</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="password" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})}
                  ...
                  placeholder="Your password" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
              </div>
