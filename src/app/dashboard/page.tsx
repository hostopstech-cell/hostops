'use client'
import { getCurrencySymbol, getDialCode, setDialCode } from '@/lib/currency-utils';
...
export default function Dashboard() {
  ...
  const handlePhoneComplete = (dialCode: string) => {
  ...
  const propRanking = properties.map(p => {
    ...
    const occ = Number(p.total_beds) > 0 ? Math.round((activeBookings / Number(p.total_beds)) * 100) : 0
