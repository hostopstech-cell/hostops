...
export default function Dashboard() {
  ...
  // Robust date parser — handles ISO strings, date-only strings, timestamps
  const getDateStr = (d: any): string => {
    ...
    try {
      ...
      // Has timezone suffix like 2026-06-10T00:00:00.000Z
  ...
  const todayRevenue = allBookings
    .filter(b => getDateStr(b.check_in) === todayStr && b.status !== 'cancelled')
    .reduce((s, b) => s + Number(b.final_amount || 0), 0)

  const totalBeds = properties.reduce((s, p) => s + Number(p.total_beds || 0), 0)
  const occupiedBeds = allBookings.filter(b => ['confirmed', 'checked_in'].includes(b.status)).length
  ...
  const arrivalsToday = allBookings.filter(b => getDateStr(b.check_in) === todayStr && b.status !== 'cancelled').length
  const departuresToday = allBookings.filter(b => getDateStr(b.check_out) === todayStr && b.status !== 'cancelled').length
  const newBookingRequests = allBookings.filter(b =>
    getDateStr(b.check_in) > todayStr && b.status !== 'cancelled'
  ).length

  // ── Revenue Chart — use check_in date properly ──
  const weekDays = Array.from({ length: 7 }, (_, i) => {
    const d = new Date()
    d.setDate(d.getDate() - (6 - i))
    const dStr = d.toLocaleDateString('en-CA')
    const rev = allBookings
      .filter(b => getDateStr(b.check_in) === dStr && b.status !== 'cancelled')
      .reduce((s, b) => s + Number(b.final_amount || 0), 0)
    return {
  ...
  const monthRevenue = allBookings
    .filter(b => getDateStr(b.check_in).startsWith(thisMonth) && b.status !== 'cancelled')
    .reduce((s, b) => s + Number(b.final_amount || 0), 0)
  ...
  // Recent Bookings: tomorrow onwards
  ...
  const propRanking = properties.map(p => {
    ...
    const activeBookings = allBookings.filter(b =>
      String(b.property_id) === String(p.id) && ['confirmed','checked_in'].includes(b.status)
    ).length
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div onClick={() => router.push('/dashboard/revenue')}
          ...
          <p className="text-xs text-slate-400 mt-2">from today's check-ins</p>
        </div>
        ...
        <div onClick={() => router.push('/dashboard/bookings?payment=pending')}
          ...
          <p className="text-xs text-slate-400 mt-2">{pendingPaymentBookings.length} booking{pendingPaymentBookings.length !== 1 ? 's' : ''} unpaid</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-1">
            <h2 className="font-bold text-slate-900">Revenue This Week</h2>
                  ...
                  <div className="w-full flex items-end" style={{ height: '112px' }}>
                    <div
                      className={`w-full rounded-t-lg transition-all duration-500 ${
                        ...
                        d.rev > 0 ? 'bg-orange-200 group-hover:bg-orange-300' :
                        'bg-slate-100'
                      }`}
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          ...
          <div className="space-y-1">
            {/* Arrivals Today */}
            <button onClick={() => router.push('/dashboard/bookings?checkin=today')}
            ...
            {/* Departures Today */}
            <button onClick={() => router.push('/dashboard/bookings?checkout=today')}
              className="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 transition-colors text-left group">
              <div className="flex items-center gap-3">
            ...
            <button onClick={() => router.push('/dashboard/bookings?filter=upcoming')}
              className="w-full flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 transition-colors text-left group">
              <div className="flex items-center gap-3">
            ...
            {/* Expected Revenue Today — Coming Soon */}
            <div className="relative overflow-hidden">
              <div className="w-full flex items-center justify-between p-2.5 rounded-xl text-left select-none">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-slate-400">Expected Revenue Today</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-bold text-slate-300">₹{(allBookings.filter(b => getDateStr(b.check_in) === todayStr && b.status !== 'cancelled').reduce((s,b) => s + Number(b.final_amount||0), 0)).toLocaleString('en-IN')}</span>
                </div>
              </div>
        ...
        <div className="lg:col-span-3 bg-slate-900 rounded-2xl p-5 shadow-sm text-white">
          <h2 className="font-bold text-white mb-4">Live Occupancy</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          {recentBookings.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-xs text-slate-300 mt-1">Tomorrow's and future check-ins will appear here</p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">By income this month</p>
            </div>
