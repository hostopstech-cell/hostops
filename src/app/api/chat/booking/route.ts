...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      guestsData, ownerId,
      available_room_ids, // new: array of {id, available_beds} from grouped booking
    } = body;
    ...
    // NEW: If available_room_ids provided (grouped booking), find first truly available room
    if (available_room_ids && Array.isArray(available_room_ids) && available_room_ids.length > 0) {
      ...
      // LEGACY: fallback — match by room name
    ...
    if (guestsData) {
      ...
      if (arr.length > 0) guestsArray = JSON.stringify(arr.map((g: any) => ({
        name: g.name || '',
        phone: g.phone || '',
