...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { guestName, guestPhone, checkIn, checkOut, amount, paymentStatus, paymentMethod, utrNumber, paymentSenderName, numberOfGuests, status, bookingSource } = body;
    ...
    await sql`
      INSERT INTO bookings (
        ...
        ${bookingSource || "direct"}, ${status || "confirmed"}, NOW()
      )
    `;
