    ...
    <div className="min-h-screen bg-white font-[family-name:var(--font-geist-sans)]">
      ...
      <nav className="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm text-slate-600">
          ...
          <Link href="/pricing" className="text-orange-600 font-semibold border-b-2 border-orange-600 pb-0.5">Pricing</Link>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              { icon: "💳", title: "No Credit Card", sub: "Required" },
              { icon: "⏱️", title: "Setup in Under", sub: "5 Minutes" },
              { icon: "❌", title: "Cancel", sub: "Anytime" },
              { icon: "⭐", title: "Full Feature", sub: "Access" },
            ].map((f) => (
              <div key={f.title} className="text-center bg-slate-50 rounded-xl p-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/login" className="flex items-center justify-center gap-2 bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">
              Start 7-Day Free Trial →
            </Link>
            <Link href="/dashboard" className="flex items-center justify-center gap-2 border border-slate-200 text-slate-700 px-6 py-3 rounded-xl font-semibold hover:bg-slate-50 transition-colors">
              ▷ View Live Demo
            </Link>
          </div>
        ...
        <div className="bg-slate-900 rounded-2xl shadow-2xl overflow-hidden hidden md:block">
          <div className="flex">
            <div className="w-36 bg-slate-950 p-3">
              ...
              {["Dashboard","Properties","Rooms & Beds","Bookings","Guests","Revenue","Reports","WhatsApp","Automation","Settings"].map((item) => (
                <div key={item} className="text-slate-400 text-xs py-1.5 px-2 rounded hover:bg-slate-800 cursor-pointer truncate">{item}</div>
              ))}
            </div>
            <div className="flex-1 p-4">
              <p className="text-slate-400 text-xs font-semibold mb-3">Dashboard Overview</p>
              <div className="grid grid-cols-3 gap-2 mb-3">
                {[{l:"Total Bookings",v:"128",c:"+12%"},{l:"Occupancy Today",v:"76%",c:"+8%"},{l:"Today's Revenue",v:"₹24,750",c:"+15%"}].map((s) => (
                  <div key={s.l} className="bg-slate-800 rounded-lg p-2">
                    <p className="text-slate-400 text-xs">{s.l}</p>
                    <p className="text-white font-bold text-sm">{s.v}</p>
                    <p className="text-green-400 text-xs">{s.c}</p>
                  </div>
                ))}
              </div>
      ...
      <section className="border-y border-slate-100 py-8 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              {icon:"⭐",v:"500+",l:"Properties Onboarded"},
              {icon:"📅",v:"10,000+",l:"Bookings Managed"},
              {icon:"👥",v:"98%",l:"Customer Satisfaction"},
              {icon:"🎧",v:"24/7",l:"Support Available"},
            ].map((s) => (
              <div key={s.l} className="flex items-center gap-3 justify-center">
                <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-lg">{s.icon}</div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan) => (
            <div key={plan.name} className={`border-2 rounded-2xl p-6 relative flex flex-col ${plan.color} ${plan.popular ? "shadow-xl scale-105" : ""}`}>
              <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-1.5 text-center mb-4">
                <p className="text-green-700 text-xs font-semibold">🎁 Includes 7-Day Free Trial</p>
              </div>
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-xs px-3 py-1 rounded-full font-semibold">Most Popular</div>
              )}
              <div className="text-3xl mb-2">{plan.icon}</div>
              <h2 className="text-xl font-bold text-slate-900">{plan.name}</h2>
              <p className="text-slate-400 text-sm mb-4">{plan.desc}</p>
              <div className="mb-1">
                <span className="text-4xl font-bold text-slate-900">₹{yearly ? plan.yearlyPrice.toLocaleString() : plan.monthlyPrice.toLocaleString()}</span>
              ...
              <Link href="/login" className={`block text-center py-3 rounded-xl font-semibold text-sm transition-colors ${plan.buttonStyle}`}>
                Start Free Trial
              </Link>
      ...
      <section className="bg-slate-50 py-16">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              {icon:"👤",n:"1",title:"Start Free Trial",desc:"Create your account and get instant access to all features."},
              {icon:"🏨",n:"2",title:"Explore & Manage",desc:"Use HostOps to manage your properties and operations."},
              {icon:"📋",n:"3",title:"Choose a Plan",desc:"Select the plan that fits your business needs."},
              {icon:"🚀",n:"4",title:"Continue & Grow",desc:"Continue seamlessly and scale your business."},
            ].map((s,i) => (
              <div key={s.n} className="text-center relative">
                {i < 3 && <div className="hidden md:block absolute top-6 left-1/2 w-full h-0.5 bg-slate-200" />}
                <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center mx-auto mb-3 relative z-10 text-2xl">{s.icon}</div>
                <p className="text-orange-600 text-xs font-semibold mb-1">{s.n}.</p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 grid md:grid-cols-2 gap-12">
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-8 flex flex-col justify-center text-center">
          ...
          <Link href="/login" className="bg-orange-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">
      ...
      <section className="border-t border-slate-100 py-8">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className="text-slate-400 text-sm mb-6">Trusted by leading property owners</p>
