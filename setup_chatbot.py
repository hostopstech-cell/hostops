...
# File 1: Chat API Route
write('src/app/api/chat/[propertyId]/route.ts', '''import { NextRequest, NextResponse } from 'next/server';
...
export async function POST(
  ...
  try {
    ...
    const properties = await sql`
      SELECT p.*,
        json_agg(DISTINCT jsonb_build_object('name', r.name, 'type', r.type, 'price', r.price_per_night, 'capacity', r.capacity)) as rooms
      FROM properties p
...
Available Rooms:
${rooms.map((r: any) => `- ${r.name} (${r.type}): Rs${r.price}/night, capacity: ${r.capacity}`).join('\\n') || 'Contact property for details'}
...
# File 2: Booking Save API
write('src/app/api/chat/booking/route.ts', '''import { NextRequest, NextResponse } from 'next/server';
import { neon } from '@neondatabase/serverless';
